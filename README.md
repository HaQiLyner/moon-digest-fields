...
把“字段是否能解析”和“消息体是否真的匹配”分成可观察的结果。

## 核心能力

- 严格解析 RFC 9530 所用 Structured Fields 字典子集，拒绝重复算法、非法 Base64、越界权重和尾随逗号；
- 对字段成员排序并生成稳定表示；
- 一次性或分块计算 SHA-256，生成标准 Digest Fields 值；
...
println(result.decision.to_string()) // verified
```
...
0.1.0 的验收范围是：四类字段的严格解析/稳定输出、SHA-256 生成与分块计算、
验证策略、算法协商、批量审计、RFC 已知向量、篡改用例、2000 条集成场景和
20000 次吞吐工作负载。明确不包含 HTTP 客户端/服务端、TLS、HTTP Message
Signatures、SHA-512 计算、内容编码解压或媒体格式规范化。调用方必须把正确的
内容（Content）或选定表示（Representation）字节交给库。

## 测试与实际场景

`scenario_test.mbt` 模拟网关审计 2000 条响应：1600 条通过、200 条正文被改动、
100 条字段损坏、100 条字段缺失，并断言分类总数。`examples/bench` 对完整的
解析与 SHA-256 验证路径执行 20000 次。测量方法与本机结果见
